    ...
    <div class="wrap-main">
        <div class="container">
            <div class="row">
                ...
                @can('Quản lý ngôn ngữ-create')
                <div class="col">
                    <form class="row g-3" method="POST"
                          action="@if(!empty($currentItem)){{route('languages.update', ['language'=>$currentItem])}} @else{{route('languages.store')}}@endif">
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea type="text" class="form-control form-control-sm form-control-sm" id="en" name="en"
                                      required>{{$currentItem->en ??''}}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="vi" class="form-label fw-bold">Vi</label>
                            <textarea type="text" class="form-control form-control-sm form-control-sm" id="vi" name="vi"
                                      required>{{$currentItem->vi ??''}}</textarea>
                        </div>
                                ...
                                <td style="width:40px">
                                    ...
                                    @can('Quản lý ngôn ngữ-update')
                                        <a class=" text-warning mr-2 d-inline-block" style="margin-right: 5px;"
                                           href="{{route('languages.edit',['language' => $item])}}">
                                        ...
                                        @endcan
                                        @can('Quản lý ngôn ngữ-delete')
                                        <form action="{{route('languages.destroy',['language' => $item])}}" method="POST">
                                            @csrf
                                            {{ method_field('DELETE') }}
