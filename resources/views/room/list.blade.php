                ...
                <li id="room-{{$room->id}}" class="position-relative @if($room->status != \App\Models\Room::NOT_FOR_RENT) booking-room @endif bg-{{$room->getBgButton()}}"
                    ...
                    @if($bookingRoomFirstOrder)
                        <div class="room" data-room-id="{{$room->id}}"
                            ...
                            <button style="background-color: #cd4c00;color: #fff;" id="btn-infor-booking-room"
                                    class="btn btn-sm">
                    ...
                    @if($bookingRoomFirstOrder)
                        <span class="d-block time small">
                                            <small>Đặ̣t phòng:
                                                <span
                                                    class="text">{{$bookingRoomFirstOrder->getTimeStartDate()}} {{$bookingRoomFirstOrder->getDateStartDate()}}</span>
                                        </small>
                                        </span>
                    @endif
                </li>
