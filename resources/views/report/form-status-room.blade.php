<div class="col-md-12">
    <div class="d-flex justify-content-end align-items-center mb-3">
        <div class="filter">
            <form action="{{route('reports.index')}}" class="d-flex" method="GET">
                <input type="text" autocomplete="off"  class="form-control form-control-sm me-2 filter-date-not-time" placeholder="{{__('Choose_date')}}" name="start_date" value="@if(!empty(request()->start_date)) {{request()->start_date}} @endif"  autocomplete="off">
    ...
    <table class="table table-sm table-bordered table-hover">
        ...
        <tr>
            ...
            <th scope="col">{{__('Total_short')}}</th>
        </tr>
        </thead>
        <tbody>
        @if(!empty($items))
